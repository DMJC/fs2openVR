...
int button_function(int n)
{
	...
	switch (n) {
		...
		case SHIELD_EQUALIZE:		// equalize shield energy to all quadrants
			// Resume ASM (if enabled) until player presses a XFER key
			Player->flags &= ~PLAYER_FLAGS_AUTO_SHIELD_MANAGEMENT_OVERRIDE;
			return button_function_critical(n);
		...
		case SHIELD_XFER_RIGHT:		// transfer shield energy to right
			// Pause ASM (if enabled) until player presses the EQUALIZE key
			Player->flags |= PLAYER_FLAGS_AUTO_SHIELD_MANAGEMENT_OVERRIDE;
			return button_function_critical(n);
	}
	...
	switch(n) {
		...
		case TOGGLE_AUTO_SHIELD_MANAGMENT:
			if (Player_use_ai) {
				gamesnd_play_iface(SND_GENERAL_FAIL);
				HUD_printf(XSTR("Cannot activate auto shield manager with autopilot on", -1));	//TODO add strings.tbl entry
				break;
			}

			Players[Player_num].flags ^= PLAYER_FLAGS_AUTO_SHIELD_MANAGEMENT;
			if (Players[Player_num].flags & PLAYER_FLAGS_AUTO_SHIELD_MANAGEMENT) {
				Players[Player_num].flags &= ~PLAYER_FLAGS_AUTO_SHIELD_MANAGEMENT_OVERRIDE;
				button_function(SHIELD_EQUALIZE);
				Player_ai->mode = AIM_MANAGE_SHIELD;
